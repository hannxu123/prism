...
model = 'gemini-2.5-pro'  # 'gpt-4.1-mini'
...
y = (df['Heart Disease'] == 1).astype(int)
...
for i in range(0, 300): # 200, 300
    ...
    for ff in columns:
        ...
        if info_df.iloc[0][tg_feature] == info_df.iloc[1][tg_feature]:
            ...
            '\n\nYour task is to evaluate the likelihood that each person will have heart disease.\n\n' + \
            ('For each patient:\n'
            ...
            response = inference_gpt(messages=[{"role": "user", "content": user_prompt}], model=model, verbose=True)
    ...
    with open("gemini_results/shap_heart_iter2.json", "w", encoding="utf-8") as f:
